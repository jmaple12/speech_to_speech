...
with gr.Blocks() as demo:
    ...
    with gr.Tab('Chat'):
        with gr.Row():
            ...
            asr_text_process = gr.Textbox(label="show asr process", show_label=False, placeholder="show asr process", scale=30)
    ...
    #版本2的tts功能，与llm并行运行
    #1.开关按钮切换；2.开关tts api，3. TTS().run启用/关闭4.use_tts_on2:查找是否成功开启tts服务，传入tts_params.json文件的参数；use_tts_close2：关闭参数组合展示||读取chatbot内容的代码在txt_submit那里
    ...
    #ASR模块交互
    #点击按钮，更改label，更改txt和txt_submit的显示隐藏,归零asr_show_len已读asr_err长度，初始化asr
    start_asr0 = start_asr.click(lambda :gr.Button("asr is ininting"), None, start_asr, queue=True,).then(lambda :[gr.Textbox(visible=False), gr.Textbox(visible=True)], None, [txt, txt_submit]).then(lambda :gr.Number(0), None, asr_show_len).then(asr_init, gr.Textbox(asr_api_path ,visible=False), None,)
    # 关闭可能的asr线程，检查asr_err是否初始化----下面这段不能写，写在asr_init也不行...
    # start_asr1 = start_asr0.then(read_ast_out, None, [asr_pid, asr_port]).then(rc.close_pid_and_port, [asr_pid, asr_port], None)
    start_asr1 = start_asr0.then(check_asr, None, [asr_pid, asr_url])
    #asr_launched，切换按钮显隐顺序，--待办：加入asr_api地址，使用dropdown
    ...
    tt_count_process = start_asr2.then(t_count, [gr.Number(value=0.8, visible=False), gr.Textbox(value=None, visible=False)], time_count)
    star_asr3 = start_asr2.then(asr_request, None, txt_submit)
    #每秒读取一次文件
    time_count.change(get_asr_process, asr_show_len, [asr_text_process,asr_show_len])
    ##txt submit成功后：先清空asr_text_process，继续请求语音转写
    chat_end_flag.change(lambda :gr.Textbox(None), None, asr_text_process).then(lambda x: asr_request() if x==1 else "", chat_end_flag, txt_submit)
    #关闭asr按钮---停止time_count的迭代，开启和关闭按钮显示隐藏切换，txt和txt_submit的显示隐藏切换，关闭asr_api的接口
    end_asr.click(lambda :[gr.Button(visible=False), gr.Button(visible=True)], None, [end_asr, start_asr],queue=True, cancels=[tt_count_process]).then(lambda x,y:rc.close_pid_and_port(x,y), [asr_pid, asr_port], None).then(lambda :[gr.Textbox(visible=False), gr.Textbox(visible=True)], None, [txt_submit,txt])
    ...
    txt_submit2 = txt_submit1.then(lambda x,y:[x+[(y, '')], gr.Textbox(value="", interactive=False)], [chatbot, txt_submit], [chatbot, txt_submit]).then(lambda:gr.Number(value=0), None, chat_end_flag,)
    ...
    #生成回复
    txt_submit4 = txt_submit3.then(bot, chatbot, chatbot, api_name="bot_response").then(lambda:gr.Number(value=1), None, chat_end_flag).then(lambda x:gr.Textbox(interactive=True), txt_submit, txt_submit, queue=False)

    #当chatbot或者chat_end_flag变化时，将内容加入到tts_read_process_json后，然后根据json内容统一单线程传入tts.tts_text
