...
class TTS():
    ...
    def tts_text(self, process_json):
        ...
        if self.open_tts:
            ...
            with open(self.cache_path+'other_params_saved.json','r') as file:
                temp_val = json.loads(file.read())
            ##chat_end_flag=1表示对话完成，直接传入chatbot最后回复的长度
            temp_val['tts_have_read_len'] = len(process_json['chatbot'][-1])
    ...
    def tts_text_open(self, process_json):
        ...
        if history:
            ...
            reply = ''
        if 'tts_have_read_len' in temp_val:
            have_read_length = temp_val['tts_have_read_len']
        else:
            temp_val['tts_have_read_len'] = 0
            open(self.cache_path+'other_params_saved.json','w+').write(json.dumps(temp_val)) 
        if self.display:
        ...
        if reply[have_read_length:]: 
            ...
            if ((reply[-1] in self.splits)&(len(text_list) >=self.endure_sentence))|(num==1):
                ...
                while ((num==1)&(len(text_list)>0))|(len(text_list) >=self.endure_sentence):
                    ...
                    temp_val['tts_have_read_len'] = have_read_length
