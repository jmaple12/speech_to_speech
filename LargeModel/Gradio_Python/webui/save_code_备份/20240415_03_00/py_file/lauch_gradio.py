...
with gr.Blocks() as demo:
    with gr.Row():
        with gr.Column(scale=1, min_width=200):
            ...
            gr.Interface(save_cut_params, inputs=[max_epoch, cut_nepoch], outputs='text', description='click to save cut_params')
            ...
            gr.Interface(save_params, inputs=[num_predict,temperature,top_p,top_k,num_ctx, repeat_penalty, seed,num_gpu,stop], outputs='text', description='click to save params', )

            #保存参数
            #展示保存的参数字典--展示键值----仅仅刷新页面并不能更新mycpp
            ...
            with gr.Row():
                ...
                refresh_params_dp = gr.Dropdown(choices=['add and cover params', 'add but not cover params', 'refresh params'], value='add but not cover params')
                refresh_params_dp.change(modify_label, inputs=[save_params_button, refresh_params_dp], outputs=[save_params_button], )
                #把键值存储到string里面
                saved_params_display_tex = gr.Textbox(visible=False)
                #点击保存--更新字典
                save_params_button.click(ccps, inputs=[saved_params_display,saved_params_display_tex, refresh_params_dp], outputs=[saved_params_display,saved_params_display_tex])
            #点击字典更改值
            ...
            with gr.Row():
                ...
                save_params_modify.submit(params_modify_textbox, inputs=[save_params_modify, saved_params_display,saved_params_display_tex], outputs=[saved_params_display,saved_params_display_tex])
                # 字典值删除    
                save_params_delete = gr.Button('click to delete key name')
                save_params_delete.click(params_delete_button, inputs=[saved_params_display,saved_params_display_tex], outputs=[saved_params_display,saved_params_display_tex])
