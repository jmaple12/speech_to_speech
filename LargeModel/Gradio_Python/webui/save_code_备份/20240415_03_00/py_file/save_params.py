...
def display_params_button(saved_params_display, model, model_loader, 
    ...
    if saved_params_display:
        ...
        max_epoch, cut_nepoch = gr.Slider.update(value=model_params['cut_params']['max_epoch']),gr.Slider.update(value=model_params['cut_params']['cut_nepoch'])
...
def ccps(saved_params_display, saved_params_display_tex, refresh_params_dp):
    ['add and cover params', 'add but not cover params', 'refresh params']
    ccptt = create_cus_params()
    add=True
    if refresh_params_dp == 'add and cover params':
        add = False
    elif refresh_params_dp == 'add but not cover params':
        add =True
    if refresh_params_dp != 'refresh params':
    ...
    return(saved_params_display, saved_params_display_tex)
...
def modify_label(save_params_button, refresh_params_dp):
    ...
    elif refresh_params_dp == 'refresh params':
        save_params_button = gr.Button('click to refresh current saved params')
    return(save_params_button)

def params_modify_textbox(save_params_modify, saved_params_display,saved_params_display_tex):
    if save_params_modify and saved_params_display:
        ...
        saved_params_display = gr.Dropdown.update(choices=temp_str.split('\n'))
    else:
        print("dont submit null string, or no params has been saved")
    return(saved_params_display, saved_params_display_tex)


def params_delete_button(saved_params_display, saved_params_display_tex):
    if saved_params_display:
        ccp = create_cus_params()
