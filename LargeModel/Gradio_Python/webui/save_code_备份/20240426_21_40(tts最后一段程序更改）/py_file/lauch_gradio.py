...
with gr.Blocks(css=main_css) as demo:
    ...
    with gr.Tab('Params'):
        ...
        with gr.Row():
            ...
            with gr.Column(min_width=400):
                ...
                #提交动画：label:显示文本已经提交
                promot_text.submit(lambda:gr.Textbox(label="text has submitted"), None, promot_text, queue=True).then(lambda :gr.Textbox(label="promot_text, enter to save"), None, promot_text)
                with gr.Row():
                    #选择框架
                ...
                #点击动画：
                save_cut_param_button.click(lambda :gr.Button('saved cut_params'), None,save_cut_param_button, queue=True).then(lambda :gr.Button('click to save cut_params'),None,save_cut_param_button,)
                # #选择最大对话轮次，超过时裁剪对话记录
            ...
            with gr.Column(min_width=100, scale=1):
                ...
                #点击动画：
                save_model_loader_params_button.click(lambda:gr.Button('saved model_loader_params'), None,save_model_loader_params_button, queue=True).then(lambda :gr.Button('click to save model_loader params'), None, save_model_loader_params_button,)
    ...
    with gr.Tab('Chat'):
        with gr.Row():
            with gr.Column(min_width=250):
                ...
                tts_bat_path_text = gr.Textbox(label="input gpt_tts bat file",value=bat_path_text_cache['tts'].strip('\'').strip('"') if bat_path_text_cache['tts'] else None,)
                ...
                with gr.Row():                  
                    ...
                    start_tts_service = gr.Number(0, label="start tts service or not", visible=False)
                #对话中，每次传入几句话给tts
    ...
    start_asr0 = start_asr.click(lambda :gr.Button("asr is ininting"), None, start_asr, queue=True,).then(lambda :[gr.Textbox(visible=False), gr.Textbox(visible=True)], None, [txt, txt_submit]).then(lambda :gr.Number(0), None, asr_show_len).then(read_ast_out, None, [asr_pid, asr_port]).then(rc.close_pid_and_port, [asr_pid, asr_port], None).then(asr_init,asr_api_path, None,)
    # 检查asr_err是否初始化----下面这段关闭可能的asr线程的不能写，写在asr_init前面也不行...
    start_asr1 = start_asr0.then(check_asr, None, [asr_pid, asr_url])
    
    #asr_launched，切换按钮显隐顺序，更新start_asr_flag的值--待办：加入asr_api地址，使用dropdown
    start_asr2 = start_asr1.then(lambda :[gr.Button("start asr", visible=False), gr.Button(visible=True)], None, [start_asr, end_asr]).then(lambda:gr.Number(value=1), None, start_asr_flag)
    ...
    #初始化tts_read_process_json和tts_have_read_len||待办：第二个then有时候会让
    ...
    stop_chat_button.click(None, None, None, cancels=[txt_submit4])

    #当chatbot变化时，将内容加入到tts_read_process_json后，然后根据tts_read_process_json内容传入tts.tts_text，每一次转换完成后，检查chat_end_flag是否为1，若为1，表示chatbot已经读写完毕，再次运行tts_text，防止前面转写后的剩余文字无法读取。||temp_tts_flag表示一小段读写完成的标志，1表示读写完成。
    temp_tts_flag = gr.Number(value=0, visible=False)
    chatbot.change(tts_process_json, [chatbot, chat_end_flag,tts_read_process_json, gr.Textbox("chatbot",visible=False), ], tts_read_process_json, queue=True).then(lambda :[gr.Number(value=0),gr.Number(value=0)], None, [tts_finish_flag,temp_tts_flag]).then(lambda xxx:tts_funcion.tts_text(xxx), tts_read_process_json, temp_tts_flag,).then(lambda xxx:print_fun('chatbot,',xxx), temp_tts_flag, None)

    chat_end_flag.change(tts_process_json, [chatbot, chat_end_flag, tts_read_process_json, gr.Textbox("chat_end_flag",visible=False), ], tts_read_process_json, queue=True).then(lambda :[gr.Number(value=0),gr.Number(value=0)], None, [tts_finish_flag,temp_tts_flag]).then(lambda xxx,yyy:tts_funcion.tts_text(xxx) if yyy==1 else 0, [tts_read_process_json,temp_tts_flag], temp_tts_flag,).then(lambda xxx:print_fun('chat_end_flag', xxx), temp_tts_flag, None)

    temp_tts_flag.change(lambda xxx:print_fun('temp_tts',xxx), temp_tts_flag, None).then(lambda xxx,yyy,zzz:tts_funcion.tts_text(yyy) if (zzz==1)&(xxx==1) else 0, [temp_tts_flag, tts_read_process_json, chat_end_flag], tts_finish_flag)
