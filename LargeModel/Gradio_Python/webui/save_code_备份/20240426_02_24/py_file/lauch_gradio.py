...
with gr.Blocks() as demo:
    ...
    with gr.Tab('Chat'):
        with gr.Row():
            with gr.Column(min_width=200):
                ...
                with gr.Row():
                    ...
                    #删除聊天记录按钮
                    delete_chat_record = gr.Button('delete this record', scale=0)
            ...
            with gr.Column(min_width=600, scale=8, visible=True):
                ...
                with gr.Row():
                    ...
                    #中断chatbot的按钮
                    stop_chat_button = gr.Button('interrupt reply', scale=0)
        ...
        with gr.Row():
            ...
            #当语音对话结束时，开启新一轮对话的按钮
            new_asr_request = gr.Button("launch asr chat", scale=0)
            #是否开启asr服务
    ...
    use_tts_close2.click(lambda x:[gr.Button(visible=False, value="tts is launching"), gr.Button(visible=True)], use_tts_on2, [use_tts_close2, use_tts_on2], queue=True).then(display_hide_audio_params, [use_tts_close2], [tts_over_all_params]).then(run_close_tts, [use_tts_close2, tts_bat_path_text], None).then(lambda :gr.Number(value=0), None, start_tts_service)
    ...
    star_asr3 = start_asr2.then(asr_request, None, txt_submit)
    ...
    chat_end_flag.change(lambda :gr.Textbox(None), None, asr_text_process).then(lambda xxx,yyy,zzz: asr_request() if (xxx==1)&(yyy==0)&(zzz==1) else "", [chat_end_flag,start_tts_service, start_asr_flag], txt_submit)
    ...
    new_asr_request.click(asr_request, None, txt_submit)
    ...
    txt.submit(lambda x:x, txt, txt_submit,queue=True).then(lambda :gr.Textbox(value=None), None, txt)
    ...
    txt_submit5 = txt_submit4.then(lambda:gr.Number(value=1), None, chat_end_flag).then(lambda x:gr.Textbox(interactive=True), txt_submit, txt_submit, queue=False)
    ...
    #当chat_end_flag为0的时候不改变tts_read_process_json，因为在txt_submit中将chat_end_flag改为0时，tts_read_process_json还没有来得及因为chatbot的改变而改变，导致有时候tts会因为chat_end_flag的改变先读取上一轮的结果；
    chat_end_flag.change(lambda xxx,yyy: tts_process_json(xxx, yyy, 'chat_end_flag') if xxx==1 else yyy, [chat_end_flag,tts_read_process_json], tts_read_process_json, queue=True)
    ...
    tts_read_process_json.change(lambda :[gr.Number(value=0),gr.Number(value=0)], None, [tts_finish_flag,temp_tts_flag]).then(tts_funcion.tts_text, tts_read_process_json, temp_tts_flag, )
    #为tts转换语音上一层保险
    temp_tts_flag.change(lambda xxx,yyy,zzz:tts_funcion.tts_text(yyy) if (zzz==1)&(xxx==1) else 0, [temp_tts_flag, tts_read_process_json, chat_end_flag], tts_finish_flag)
