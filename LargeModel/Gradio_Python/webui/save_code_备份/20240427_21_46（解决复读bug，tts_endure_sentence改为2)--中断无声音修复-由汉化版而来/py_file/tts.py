...
class TTS():
    def __init__(self, default_model_path =default_model_path, params=default_params, default_params=default_params, play=True, audio_save_file='', tts_api_path=default_tts_api_path, cache_path='./', splits=splits, endure_sentence=2,display=False):
    ...
    def tts_text(self, process_json):
        ...
        '''
        if self.display:
                print("tts.py是否开启open_tts:",self.open_tts)
        if self.open_tts:
