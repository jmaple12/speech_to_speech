...
def interrupt_rerun(use_tts_close2, ):
    if display:
        print("行动之前，tts是否开启：",tts_funcion.open_tts)
    if use_tts_close2 == "tts is launching":
        tts_funcion.open_tts = False
    else:
        tts_funcion.open_tts = True
    print("行动之后，tts是否开启：",tts_funcion.open_tts)
...
with gr.Blocks(css=main_css) as demo:
    ...
    with gr.Tab('Chat'):
        with gr.Row():
            with gr.Column(min_width=200):
                ...
                #对话中，每次传入几句话给tts
                tts_endure = gr.Slider(minimum=1, maximum=20, step=1, value=2, label="modify tts_edure_sentence")
        ...
        with gr.Column(visible=False) as tts_over_all_params:
            ...
            with gr.Row():
                ...
                text_split_method_show = gr.Dropdown(text_split_method_choices, label='text split method', value= text_split_method_choices[int(tts_params_default['params']['text_split_method'][3:])])
                ...
                prompt_lang = gr.Dropdown(['zh','en','ja'],label='prompt_lang', value=tts_params_default['params']['prompt_lang'])
                #文本语言
    ...
    #中断回复, cancels只会中断action中的head---中断也可以恢复了
