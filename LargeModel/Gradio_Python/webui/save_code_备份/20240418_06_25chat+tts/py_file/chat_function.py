...
def model_load_weight(chat_record):
    if os.path.exists(cache_path+'record_time.txt'):
        with open(cache_path+'record_time.txt', 'a+', encoding='utf-8') as file:
            file.write(f'{chat_record}\n')
    else:
        with open(cache_path+'record_time.txt', 'w+', encoding='utf-8') as file:
            file.write('None\n')
