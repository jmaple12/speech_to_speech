...
with gr.Blocks(css=main_css) as demo:
    ...
    with gr.Tab('LLM 参数'):
        ...
        with gr.Row():
            ...
            with gr.Column(min_width=400):
                ...
                #提交动画：label:显示文本已经提交
                promot_text.submit(lambda:gr.Textbox(label="文本已提交"), None, promot_text,).then(lambda :t_count(2,2),None, None).then(lambda :gr.Textbox(label="输入Promot 文本，按Enter保存"), None, promot_text)
                with gr.Row():
                    #选择框架
                ...
                #点击动画：
                save_cut_param_button.click(lambda :gr.Button('截断参数已保存'), None,save_cut_param_button, ).then(lambda :gr.Button('点击保存聊天记录截断参数'),None,save_cut_param_button,)
                # #选择最大对话轮次，超过时裁剪对话记录
            ...
            with gr.Column(min_width=100, scale=1):
                ...
                #点击动画：
                save_model_loader_params_button.click(lambda:gr.Button('model_loader参数已保存'), None,save_model_loader_params_button, ).then(lambda :gr.Button('点击保存model_loader参数'), None, save_model_loader_params_button,)
    ...
    with gr.Tab('聊天'):
        with gr.Row():
            ...
            with gr.Column(min_width=200):
                ...
                with gr.Row():
                    ...
                    asr_text_process = gr.TextArea(label="ASR进度转写记录", show_label=False, placeholder="ASR进度转写记录", scale=30, container=False, value='')
                ...
                #asr_api地址
                asr_api_path = gr.Textbox(label='ASR API(.bat)文件位置', value=bat_path_text_cache['asr'] if bat_path_text_cache['asr'] else None,)
        ...
        if os.path.exists(cache_path+'tts_params.json'):
            with open(cache_path+'tts_params.json','r') as tts_p_file:
                temp = tts_p_file.read()
                if temp:
                    tts_p_file = json.loads(temp)
                else:
                    tts_p_file = dict()
    ...
    #点击重启ollama
    restart_ollama.click(lambda :gr.Button("重启中...请等待3秒"), None, restart_ollama, ).then(rc.clear_rerun_ollama, None, None).then(lambda :gr.Button("重启Ollama APP"), None, restart_ollama, )
    #存储promot_text
    promot_text.submit(save_promot_text, inputs=[promot_text], outputs=None, ).then(model_run.add_promot, None, None,)
    ...
    #存储model值
    model.change(save_model, inputs=[model], outputs=None, ).then(model_run.params_update, None,None,)
    #根据max_epoch选项更改cut_epoch的值
    max_epoch.change(cut_epoch, inputs=[max_epoch, cut_nepoch], outputs=[cut_nepoch])
    #保存cut_params到本地cache
    save_cut_param_button.click(save_cut_params, inputs=[max_epoch, cut_nepoch], outputs=None, ).then(model_run.params_update, None, None,)
    #保存params到本地cache
    save_model_loader_params_button.click(save_params, inputs=[num_predict,temperature,top_p,top_k,num_ctx, repeat_penalty, seed,num_gpu,stop], outputs=None, ).then(model_run.params_update, None, None,)
    #将各种参数保存到特定路径，并展示
    ...
    saved_params_display.change(display_params_button, inputs=[saved_params_display, model, model_loader, 
        ...
        num_predict,temperature, top_p, top_k, num_ctx,repeat_penalty, seed, num_gpu,stop, max_epoch, cut_nepoch, promot_text], ).then(model_run.params_update, None, None,)
    #重命名参数组名称
    ...
    save_chat_button.click(model_run.save_history_to_cache, None, None, ).then(json_update, [save_chat_json], [save_chat_json])
    ...
    chat_record_fn1=chat_record.change(json_update, [save_chat_json], [save_chat_json], ).then(lambda xxx,yyy:refresh_chat_end_flag(yyy[xxx][0]), [chat_record, save_chat_json], None,)
    chat_record_fn1.then(display_record_chat, [chat_record, save_chat_json, chatbot], [chatbot],)
    #保存chat_record键值到本地，更新对话的messages记录
    chat_record.select(model_load_weight, chat_record, None, ).then(model_run.load_messages, None, None,)
    #当json仅有一个key时候chat_record blur时自动显示----不太行，待办

    #清空chatbot, 存入is_clear_chat值，以待model读取
    clear_button.click(clear_inform, inputs=[chatbot], outputs=[chatbot], ).then(model_run.clear, None, None,)
    ...
    use_tts_on2.click(lambda:[gr.Button(visible=False), gr.Button(visible=True)], None, [use_tts_on2, use_tts_close2], ).then(run_close_tts, [use_tts_on2, tts_bat_path_text], None).then(check_tts, None, use_tts_close2).then(lambda :gr.Number(value=1), None, start_tts_service, )

    use_tts_close2.click(lambda :[gr.Button(visible=False, value="TTS开启中"), gr.Button(visible=True)], None, [use_tts_close2, use_tts_on2], ).then(display_hide_audio_params, [use_tts_close2], [tts_over_all_params]).then(run_close_tts, [use_tts_close2, tts_bat_path_text], None).then(lambda :gr.Number(value=0), None, start_tts_service, )

    # #点击按钮audio_params_submit将自定义tts参数保存到本地并传入tts_api,并更改自身的label值，再改回来label值
    audio_params_submit.click(lambda :gr.Button("TTS参数已保存并激活"), None,audio_params_submit, ).then(params_overall_json_update, [audio_params_json, gpt_path,sovits_path,ref_audio_path, prompt_text, batch_size,prompt_lang,text_lang,audio_seed,text_split_method,speed_factor], [audio_params_json]).then(lambda tts_json:open(cache_path+'tts_params.json','w+').write(json.dumps(tts_json)), audio_params_json, None).then(lambda tts_json:tts_funcion.modify(tts_json['model_path'], tts_json['params']), audio_params_json, None).then(lambda :gr.Button("保存并应用TTS参数"), None, audio_params_submit,)
    ...
    start_asr0 = start_asr.click(lambda :gr.Button("ASR启动中"), None, start_asr, ).then(lambda :[gr.Textbox(visible=False), gr.Textbox(visible=True)], None, [txt, txt_submit]).then(lambda :gr.Number(0), None, asr_show_len).then(read_ast_out, None, [asr_pid, asr_port]).then(rc.close_pid_and_port, [asr_pid, asr_port], None).then(asr_init,asr_api_path, None,)
    # 检查asr_err是否初始化
    ...
    star_asr3 = start_asr2.then(lambda :begin_next_epoch_asr(), None, txt_submit)

    #当读到info的时候，找到之前最后一轮转译结果
    temp_process_asr = gr.Textbox(value='', visible=False)
    #每0.5秒读取一次文件
    time_count.change(get_asr_process, [asr_show_len, asr_text_process, temp_process_asr], [asr_text_process,asr_show_len, temp_process_asr], show_progress='hidden')
    ...
    #txt_submit提交成功后：若chat_end_flag=1，先清空asr_text_process，当tts服务未开启时，asr服务开启时，请求语音转写
    chat_end_flag.change(lambda xxx,yyy,zzz:['',''] if xxx else [yyy,zzz], [chat_end_flag, asr_text_process, temp_process_asr], [asr_text_process,temp_process_asr]).then(lambda xxx,yyy,zzz: begin_next_epoch_asr() if (xxx==1)&(yyy==0)&(zzz==1) else "", [chat_end_flag,start_tts_service, start_asr_flag], txt_submit)

    #手动开启新的一轮语音对话，即发起asr_request
    new_asr_request.click(lambda :begin_next_epoch_asr(), None, txt_submit)

    #关闭asr按钮---停止time_count的迭代，开启和关闭按钮显示隐藏切换，txt和txt_submit的显示隐藏切换，关闭asr_api的接口，将start_asr_flag归0
    end_asr.click(lambda :[gr.Button(visible=False), gr.Button(visible=True)], None, [end_asr, start_asr], cancels=[tt_count_process]).then(lambda x,y:rc.close_pid_and_port(x,y), [asr_pid, asr_port], None).then(lambda :[gr.Textbox(visible=False), gr.Textbox(visible=True)], None, [txt_submit,txt]).then(lambda:gr.Number(value=0), None, start_asr_flag, )

    #提交文本并生成对话。防止选择聊天记录的时候，tts_have_read_len里的旧值小于chatbot的原记录--(第一交互是冗余，只是害怕submit想不到的情况，所以暂时不改)。令tts_have_read_len是上一轮对话的回复长度
    txt.submit(lambda x:x, txt, txt_submit,).then(lambda :gr.Textbox(value=None, interactive=False), None, txt, )
    ...
    txt_submit1 = txt_flag.change(lambda josn:refresh_chat_end_flag(josn['chatbot']), tts_read_process_json, None, )
    ...
    txt_submit5 = txt_submit4.then(lambda:gr.Number(value=1), None, chat_end_flag).then(lambda :[gr.Textbox(interactive=True),gr.Textbox(interactive=True)], None, [txt_submit, txt], )
    ...
    tts_read_process_json1 = tts_read_process_json.change(lambda :[gr.Number(value=0),gr.Number(value=0)], None, [tts_finish_flag,temp_tts_flag],)
    ...
    #中断回复, cancels只会中断action中的head---cancels删除一个就行了！！！
    stop_chat_button.click(lambda :tts_funcion.tts_text({'chatbot':[], 'chat_end_flag':0}), None, tts_finish_flag, cancels=[txt_submit4, temp_tts_flag1],).then(refresh_chat_end_flag, chatbot, None).then(interrupt_rerun, use_tts_close2, None)
